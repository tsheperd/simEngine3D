...
title("Static Particle No Constraints: O Global Position");
xlabel("t (s)");
ylabel("position (m)");
legend('x','y','z');
hold off;
...
title("Static Particle No Constraints: O Global Velocity");
xlabel("t (s)");
ylabel("velocity (m/s)");
legend('x','y','z');
hold off;
...
title("Static Particle No Constraints: O Global Acceleration");
xlabel("t (s)");
...
plot(simulation.t,simulation.t*0,'-.');
plot(simulation.t,simulation.t*0,'--');
title("Static Particle: Quaternian Components (Analytical)");
xlabel("t (s)");
ylabel("Quaternian (-)");
legend('1','2','3','4');
ylim([-1,2]);
hold off;
if SAVE_PLOTS
	saveas(gcf,'staticParticle_Quaternian_Plot_Analytical.png');
end
